...
/**
 * The mod_assessment submission updated event.
 *
...
/**
 * The mod_assessment submission updated event class.
 *
 * @package    mod_assessment
 * @since      Moodle 2.7
 * @copyright  2014 Andy Chan
...
class submission_updated extends \core\event\base {
    ...
    public function get_description() {
        ...
        return "The user with id '$this->userid' updated the submission of student ($grade_target) in the assessment activity
    ...
    protected function get_legacy_logdata() {
        ...
        $legacylogdata = array($this->courseid,
            'assessment',
            'update submission',
            'view_submission.php?id='.$this->contextinstanceid.'&a='.$this->objectid.$param,
            $this->objectid,
    ...
    public static function get_name() {
        return get_string('eventsubmissionupdated', 'mod_assessment');
    }
    ...
    public function get_url() {
        $param = array('id'=>$this->contextinstanceid, 'a'=>$this->objectid);
        if ($this->other['userid']) {
            $param['userid'] = $this->other['userid'];
        } else if ($this->other['groupid']) {
            $param['groupid'] = $this->other['groupid'];
